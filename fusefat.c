...
int main(int argc, char *argv[])
{
	...
	if (image_file_is_hdf(pathname)) {
		fprintf(stderr, "HDF image file found\n");
		using_hdf = 1;
		res = hdf_image_open(&vol_container, pathname, (rorwplus==FLRWPLUS)?IMAGE_FILE_WRITE_ACCESS_FLAG:0);
	} else {
		fprintf(stderr, "Raw image file found\n");
		using_hdf = 0;
		res = raw_image_open(&vol_container, pathname, (rorwplus==FLRWPLUS)?IMAGE_FILE_WRITE_ACCESS_FLAG:0);
	}
	if (res) return -1;
