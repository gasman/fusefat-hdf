...
#define readvol(v,pos,off,buf)	(v)->read(v, pos, off, buf)
#define writevol(v,pos,off,buf)	(v)->write(v, pos, off, buf)
...
/*	read()		*/
  ...
  if ( (Res = readvol(V->volContainer, fatoffset, &Entry, 4)) != 4) {
...
/*	write()		*/
	...
	Value=EFD(Value);
  if ( (Res = writevol(V->volContainer, off, (char*) &Value, 4)) != 4) {
...
static int libfat_determine_fattype(Volume_t *V) {
    ...
    if ((res = memcmp( (char *) V->Bpb.BS_FilSysType, type, 8)) == 0) {
        ...
        if ( (res = readvol(V->volContainer, (off64_t) fsi_offset, &(V->Fsi), sizeof(FSInfo_t))) != sizeof(FSInfo_t) ) { perror("FSI sreadn() error"); return -1; }
...
static int libfat_initialize_fat(Volume_t *V) {
	...
	if ( (res = readvol(V->volContainer, V->rsvdbytecnt, V->fat, fatsz)) != fatsz) {
...
int	fat_partition_init(Volume_t *V, volume_container *vol_container, int flags) {	// todo: add uid and gid;
...
/*  Variables we need to read from Boot Sector  
  ...
  ------------------------------------------  */
	...
	V->volContainer = vol_container;

	if ( (res = readvol(V->volContainer, 0, &(V->Bpb), sizeof(Bpb_t))) != sizeof(Bpb_t))		//we read directly from da disk (NB FAT32 BPB)
...
/* 	--------------------------------------------------------------------		*/
    ...
    /* check BPB_BytsPerSec (expected 512) as a rudimentary test that this is indeed FAT */
    if (V->Bpb.BPB_BytsPerSec & 0xff != 0x00) { /* fail if it isn't a multiple of 256 */
        fprintf(stderr, "Volume does not appear to be FAT (bytes per sector = %d, 512 expected)\n", V->Bpb.BPB_BytsPerSec);
        return -1;
    }
...
int fat_fat_sync(Volume_t *V) {
    ...
    // lets write down FSInfo Sector
        ...
        res = writevol(V->volContainer, (off64_t) fsioff, (char *)  &(V->Fsi), sizeof(FSInfo_t));
        if (res < 0) {
            fprintf(stderr,"writen() error, line:%d\n",__LINE__);
            return -1;
        }
		...
		for (i=0; i < V->numfats; i++) {
		    ...
		    if ( (res = writevol(V->volContainer, off, V->fat, size)) != size) {
...
//				return -1;
		...
		if (i >= count) {	// we need to fetch another piece of fat.
			...
			res = readvol(V->volContainer, (off64_t) freeoff, buf, (size_t) (count * sizeof(DWORD)));
			if (res < 0) {
...
int fetch_entry(Volume_t *V, DWORD *Cluster, DWORD *Offset, LfnEntry_t *D) {
    ...
    /*	Reading the dirent	*/
    if ( (res = readvol(V->volContainer, off, D, 32)) != 32) {
...
int fetch_next_direntry(Volume_t *V, DirEnt_t *D, DWORD *Cluster, DWORD *Offset) {
	...
	if ( ! (LIBFAT_DIRENT_ISSFN(res)) ) {
		...
		/*	Checking cluster boundaries to see if we reached end of the cluster	*/
			...
			if ( (res = readvol(V->volContainer, off, &buf, 1)) != 1) {
...
//	fprintf(stderr,"off: %u, bytleft %d, count: %d\n",*Offset, byteleftperclus,count);
	...
	while (count > 0) {
		...
		res = readvol(V->volContainer, (off64_t) off, &(buf[dataread]), (size_t) numbytes);
		if (res < 0) {
			fprintf(stderr,"readn() error, line:%d\n",__LINE__);
			return -1;
		}
...
int fat_write_data(Volume_t *V, File_t *F, DWORD *Cluster, DWORD *Offset, char *buf, int count ) {
	...
	while (cnt > 0) {
		...
		res = writevol(V->volContainer, (off64_t) off, &(buf[datawritten]), (size_t) numbytes);
...
int fat_update_file(File_t *F) {
	...
	res = writevol(F->V->volContainer, (off64_t)  F->D.direntoff, (char *) F->DirEntry, (size_t) sizeof(DirEntry_t));
	if (res < 0) {
...
static off64_t fat_find_lfnslots(Volume_t *V, File_t *dir, DWORD *Cluster , DWORD *Offset, int n) {
  ...
  if ((V->FatType == FAT32) || (rootdirflag !=1)) {	// FAT32 or not rootdir
	...
	if ((res = readvol(V->volContainer, V->rootdir16off ,cluster, (EFW(V->Bpb.BPB_RootEntCnt) * 32) )) != (EFW(V->Bpb.BPB_RootEntCnt) * 32)) {
...
int fat_create(Volume_t *V, File_t *parent, char *filename , DirEntry_t *sfn, mode_t mode, int dirflag) {
	...
	if ((V->FatType == FAT32)  || (rootdirflag != 1)) {
		...
		if ((res = writevol(V->volContainer, res64, (char *) entry,((slotnum + 1) * sizeof(DirEntry_t)))) != ((slotnum + 1) * sizeof(DirEntry_t))) {
...
static int fat_real_delete(File_t *F, int dir, int flag) {
	...
	if (F->V->FatType == FAT32) {
		...
		if ((res = writevol(F->V->volContainer, res64, (char *) D.entry, (D.len * sizeof(DirEntry_t)))) != (D.len * sizeof(DirEntry_t))) {
